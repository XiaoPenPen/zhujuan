...
public class RedisUtil {
     ...
     *
     * @param key  键
     * @param time 时间（秒），time要大于0 timeUnit
     * @return
     */
    public boolean expire(String key, long time, TimeUnit timeUnit) {
        ...
        try {
            if (StringUtils.isNotBlank(key) && time > 0) {
                result = this.redisTemplate.expire(key, time, timeUnit);
            }
        } catch (Exception ex) {
            log.error(ex.getMessage(), ex);
        }
    ...
    public void remove(Set<String> set){
        set.forEach(key -> {
            long count = remove(key);
            if (count >= 1){
                log.debug("删除redis key成功! key={}", key);
            } else {
                log.debug("删除redis key失败了! key={}", key);
            }
        });
    }
    ...
    public Set<String> keys(String pattern) {
        ...
        try {
            if (StringUtils.isNotBlank(pattern)) {
                t = (Set<String>) this.redisTemplate.keys(pattern);
            }
        } catch (Exception ex) {
            log.error(ex.getMessage(), ex);
        }
    ...
    public String getStr(String key) {
        ...
        try {
            if (StringUtils.isNotBlank(key)) {
                t = StrUtil.toString(this.redisTemplate.opsForValue().get(key));
            }
        } catch (Exception ex) {
            log.error(ex.getMessage(), ex);
        }
     ...
     * @return
     */
    public <T> boolean rightPush(String key, T value) {
        try {
    ...
    /**
     * 向 redis 中放入指定 key，并设置数组类型为 list，将 value 加入到 list 头部
     *
     * @param key   键，不能为null
     * @param value 值，不能为null
     * @return
    ...
    public <T> boolean leftPush(String key, T value) {
        try {
            ...
            log.error("key={}, value={}", key, JSONUtil.toJsonStr(value));
            log.error(ex.getMessage(), ex);
            return false;
        }
    }
     ...
     * @return
     */
    public boolean lcover(String key, List<?> value) {
        try {
     ...
     * @return 移除的个数
     */
    public <T> Object rightPop(String key) {
        try {
    ...
    public <T> Object popRange(String key, long start, long end) {
        try {
            List list = this.redisTemplate.opsForList().range(key, start, end);
            List list2 = this.redisTemplate.opsForList().range(key, start, lsize(key));
            remove(key);
            this.redisTemplate.opsForList().rightPushAll(key, list2);
            return list;
        } catch (Exception ex) {
            log.error(ex.getMessage(), ex);
            return 0;
    ...
    /**
     * 从 redis 中指定保留 key 对应的 start开头 end结尾 删除该范围之外的数据
     *
     * @param key   键，不能为null
     * @return 移除的个数
     */
    ...
    public List<JSONObject> getObjs(String patten){
        Set<String> keySet = this.keys(patten);
    ...
    public List<JSONObject> getObjsOrder(String patten, String order){
        Set<String> keySet = this.keys(patten);
        List<JSONObject> list = new ArrayList<>();
        keySet.forEach(key -> {
            list.add(this.get(key));
        ...
        if (StrUtil.isNotBlank(order)){
            list.sort(Comparator.comparing(m -> StrUtil.toString(m.get(order))));
